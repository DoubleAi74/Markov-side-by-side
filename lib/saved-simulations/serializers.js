...
export function serializeDiscreteTimeState({
  components,
  generations,
  numSims,
}) {
        ...
        ? components.map((component) => ({
            ...
            outcomes: Array.isArray(component?.outcomes)
              ? component.outcomes.map((outcome) => ({
                  offspring: Number(outcome?.offspring),
  ...
  const components =
    Array.isArray(payload?.components) && payload.components.length > 0
      ? payload.components.map((component) => ({
          ...
          outcomes:
            Array.isArray(component?.outcomes) && component.outcomes.length > 0
              ? component.outcomes.map((outcome) => ({
                  id: makeClientId(),
              ...
              : [
                  { id: makeClientId(), offspring: "", probability: "" },
                  { id: makeClientId(), offspring: "", probability: "" },
                ],
          noteEnabled: Boolean(component?.noteEnabled),
          noteLabel:
