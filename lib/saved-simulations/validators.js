...
const MAX_DISCRETE_OUTCOMES = 100;
...
function normalizeDiscreteTimeComponents(value) {
  ...
  return value.map((component, index) => {
    ...
    assert(
      ...
      `payload.components[${index}].outcomes must be an array.`,
    );
    assert(
      item.outcomes.length >= 1 &&
        item.outcomes.length <= MAX_DISCRETE_OUTCOMES,
      `payload.components[${index}].outcomes must contain between 1 and ${MAX_DISCRETE_OUTCOMES} entries.`,
    );

    const outcomes = item.outcomes.map((outcome, outcomeIndex) => {
      const normalizedOutcome = requireObject(
        outcome,
        `payload.components[${index}].outcomes[${outcomeIndex}]`,
      );
      const probability = normalizeFiniteNumber(
        normalizedOutcome.probability,
        `payload.components[${index}].outcomes[${outcomeIndex}].probability`,
      );
      assert(
        probability >= 0 && probability <= 1,
        `payload.components[${index}].outcomes[${outcomeIndex}].probability must be between 0 and 1.`,
      );
      return {
        offspring: normalizeInteger(
          normalizedOutcome.offspring,
          `payload.components[${index}].outcomes[${outcomeIndex}].offspring`,
          { min: 0, max: 1000000 },
        ),
        probability,
      };
    ...
    assert(
      ...
      `payload.components[${index}].outcome probabilities must total 1.`,
    );
    assert(
      new Set(outcomes.map((outcome) => outcome.offspring)).size ===
        outcomes.length,
      `payload.components[${index}].offspring outcomes must be unique.`,
    );

    return {
      name: typeof item.name === "string" ? item.name.trim() : "",
      init: normalizeInteger(
        item.init,
        `payload.components[${index}].init`,
        { min: 0, max: 1000000 },
      ),
