  ...
  return {
    components: Array.isArray(payload?.components)
      ? payload.components.map((component) => ({
          name: trimString(component?.name),
          initial: Number(component?.init ?? 0),
          mode: component.mode ?? "branching",
          ...(component.mode === "matrix"
            ? {
                states: component.states.map(Number),
                matrix: component.matrix.map((row) => row.map(Number)),
              }
            : {
                outcomes: (component.outcomes ?? []).map((outcome) => ({
                  [component.mode === "increments" ? "change" : "offspring"]:
                    Number(component.mode === "increments" ? outcome.change : outcome.offspring),
