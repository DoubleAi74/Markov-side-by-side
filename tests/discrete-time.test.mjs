...
import {
  ...
  DiscreteTimeStepper,
} from "../components/simulators/discrete-time/engine.js";
import {
  normalizeDiscreteComponent,
  parseDiscreteComponents,
  updateOutcomeProbability,
} from "../lib/discrete-time/model.js";
import {
...
const run = (components, steps = 4, random = () => 0.25) =>
  new DiscreteTimeStepper(
    components.map(
      (component) =>
        new DiscreteTimeComponent(
          component.name,
          component.outcomes,
          component,
        ),
...
const payload = (components) => ({
  components,
  settings: { generations: 4, numSims: 1 },
});
...
test("legacy models remain branching processes with extinction absorbing", () => {
  ...
  assert.deepEqual(run([branching], 3, () => 0.9).history, [
    [3],
    [0],
    [0],
    [0],
...
test("a random walk draws once per step, crosses zero and preserves negative states", () => {
  ...
  assert.deepEqual(run([walk], 4, () => draws.shift()).history, [
    [0],
    [-1],
    [-2],
    [-1],
...
test("p = 0 and p = 1 give deterministic walks", () => {
  for (const p of [0, 1]) {
    const component = {
      ...walk,
      outcomes: [
        { change: 1, probability: p },
        { change: -1, probability: 1 - p },
      ],
    };
    assert.deepEqual(
      run([component], 3).history.flat(),
      p ? [0, 1, 2, 3] : [0, -1, -2, -3],
    );
  }
});
...
test("arbitrary jumps and a holding probability are supported", () => {
  const component = {
    ...
    init: -3,
    outcomes: [
      { change: -2, probability: 0.2 },
      { change: 0, probability: 0.3 },
      { change: 5, probability: 0.5 },
    ],
  };
  const draws = [0.1, 0.3, 0.8];
  assert.deepEqual(
    run([component], 3, () => draws.shift()).history.flat(),
    [-3, -5, -5, 0],
  );
});
...
test("a biased walk has the expected drift with a fixed random stream", () => {
  ...
  const component = {
    ...walk,
    outcomes: [
      { change: 1, probability: 0.75 },
      { change: -1, probability: 0.25 },
    ],
  };
...
test("a matrix can have absorbing states and use asymmetric probabilities", () => {
  ...
  assert.deepEqual(
    run([component], 3, () => 0.3).history.flat(),
    [-4, 9, 9, 9],
  );
});

test("multiple variables can use different transition types", () => {
  assert.deepEqual(run([branching, walk, matrix], 2).history, [
    [3, 0, -4],
    [6, 1, 9],
    [12, 2, 100],
  ]);
...
test("invalid probabilities and states fail before simulating", () => {
  const invalid = [
    ...
    { ...walk, outcomes: [{ change: 1.5, probability: 1 }] },
    {
      ...walk,
      outcomes: [
        { change: 1, probability: 0.3 },
        { change: 1, probability: 0.7 },
      ],
    },
    { ...walk, outcomes: [{ change: 1, probability: 0.8 }] },
    {
      ...walk,
      outcomes: [
        { change: 1, probability: 1.1 },
        { change: 0, probability: -0.1 },
      ],
    },
    { ...matrix, init: 0 },
  ...
  for (const component of invalid) {
    ...
    assert.throws(
      () =>
        validateSavedSimulationPayload("discrete-time", payload([component])),
      ValidationError,
    );
  }
});
...
test("linked probabilities preserve a sum of one from either numeric field", () => {
  ...
  assert.deepEqual(
    updateOutcomeProbability(outcomes, 0, "0.731", true).map(
      (outcome) => outcome.probability,
    ),
    ["0.731", "0.269"],
  );
  assert.deepEqual(
    updateOutcomeProbability(outcomes, 1, "1", true).map(
      (outcome) => outcome.probability,
    ),
    ["0", "1"],
  );
  assert.equal(
    updateOutcomeProbability(outcomes, 0, "0.2", false)[1].probability,
    0.5,
  );
  assert.equal(
    updateOutcomeProbability(outcomes, 0, "", true)[1].probability,
    0.5,
  );
  assert.equal(
    updateOutcomeProbability(
      [...outcomes, { change: 0, probability: 0 }],
      0,
      "0.3",
      true,
    )[1].probability,
...
for (const component of [branching, walk, matrix]) {
  test(`${component.mode ?? "legacy branching"} survives hydration, saving, validation and JSON export`, () => {
    ...
    const validated = validateSavedSimulationPayload(
      "discrete-time",
      saved.payload,
    );
    assert.deepEqual(
      validated.components[0],
      normalizeDiscreteComponent(original),
    );
    ...
    assert.deepEqual(
      run(parseDiscreteComponents(reopened.components)).history,
      run([original]).history,
    );
    ...
    assert.equal(
      exported.model.components[0].mode,
      original.mode ?? "branching",
    );
    ...
    if (original.mode === "matrix") {
      ...
      assert.deepEqual(
        exported.model.components[0].outcomes,
        original.outcomes,
      );
...
test("blank numeric fields cannot be silently saved as zeros", () => {
  for (const invalid of [
    ...
    { ...walk, outcomes: [{ change: "", probability: 1 }] },
    {
      ...walk,
      outcomes: [
        { change: 0, probability: 1 },
        { change: 1, probability: "" },
      ],
    },
    {
      ...matrix,
    ...
    const saved = serializeDiscreteTimeState({
      components: [invalid],
      generations: 4,
      numSims: 1,
    });
    assert.throws(
      () => validateSavedSimulationPayload("discrete-time", saved.payload),
      /required/,
    );
  }
});
...
test("duplicate variable names are rejected by both run and save validation", () => {
  ...
  assert.throws(
    () =>
      validateSavedSimulationPayload("discrete-time", payload([walk, walk])),
    /unique/,
  );
});
