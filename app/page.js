    ...
    <div className="max-w-5xl mx-auto px-4 pt-12 pb-16">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-slate-500 max-w-2xl mx-auto">
          Interactive tools for simulating stochastic processes directly in the browser.
          No installation required — define your model and run.
        </p>
      </div>

      {/* Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {SIMULATORS.map(({ href, title, subtitle, description, badge, badgeColor, accent, preset }) => (
          <Link
            key={href}
            href={href}
            className={`group bg-white rounded-xl shadow-sm border border-slate-200 border-t-4 ${accent} p-6 hover:shadow-md transition-shadow flex flex-col`}
          >
